...
> **为什么合并**：M7 的异步 mailbox 是 M6 真正异步的前提。分开做 M6 只能是同步阻塞版，
> 意义不大。合并后一次做到和 Claude Code 一样的异步非阻塞效果。
...
sequenceDiagram
    ...
    Note over Leader,Worker_B: M5 现在：同步阻塞
    用户->>Leader: "分别研究 Python 和 Rust"
    Leader->>Worker_A: await run_agent_turn("研究Python")
    Note over Leader,Worker_A: ⛔ Leader 死等
    Worker_A-->>Leader: "Python 结论"
    Leader->>Worker_B: await run_agent_turn("研究Rust")
    Note over Leader,Worker_B: ⛔ Leader 再次死等
    Worker_B-->>Leader: "Rust 结论"
    Leader->>用户: 综合回答
```
...
sequenceDiagram
    ...
    Note over Leader,Worker_B: M6+M7 目标：异步非阻塞（对标 Claude Code）
    用户->>Leader: "分别研究 Python 和 Rust"
    Leader->>Worker_A: create_task("研究Python") → 立即返回 w1
    Leader->>Worker_B: create_task("研究Rust")  → 立即返回 w2
    Note over Leader: ✅ 两个 worker 后台并行，Leader 不阻塞

    par Worker A 后台运行
        Worker_A->>Worker_A: run_agent_turn() ReAct 循环
        Worker_A->>Mailbox: send(task-notification w1 Python结论)
    and Worker B 后台运行
        Worker_B->>Worker_B: run_agent_turn() ReAct 循环
        Worker_B->>Mailbox: send(task-notification w2 Rust结论)
    end

    Mailbox-->>Leader: 注入 task-notification w1（伪用户消息）
    Mailbox-->>Leader: 注入 task-notification w2（伪用户消息）
    Leader->>用户: 综合两份结论回答
```

**目标**：leader 异步派 worker，worker 后台跑完通过 mailbox 把 `<task-notification>`
注入 leader 消息队列，leader 下一轮循环自然读到并综合。全程非阻塞。

**🎮 你能体验到**：
- `uv run orchestra coordinator`
- leader 派出 2 个 worker **立刻返回**（不等），worker 后台并行跑
- worker 跑完，`<task-notification>` 自动出现在 leader 对话里
- leader 综合两份结论回答用户
- worker 尝试越权调 `spawn_worker` → 被拒

**对标 Claude Code**：
- `coordinator/coordinatorMode.ts`（角色分工 + worker 工具收窄）
- `utils/mailbox.ts`（极简异步队列，原文 73 行）
- `tools/SendMessageTool/`（网状通信）
- `tasks/LocalAgentTask/LocalAgentTask.tsx:197`（`<task-notification>` XML 格式）

**要写**：

`mailbox.py`（新建，替换 `raise NotImplementedError`）
- `Mailbox`：`asyncio.Queue` + waiters 列表
- `send(msg)`：有人等就唤醒，否则入队
- `receive()`：阻塞等待
- `poll()`：非阻塞瞅一眼

`context.py`（小改）
- `RunContext` 加 `mailbox: Mailbox`（自己的收件箱）
- `RunContext` 加 `task_registry: TaskRegistry`（管理派出去的 worker）

`coordinator.py`（主体，替换 `raise NotImplementedError`）
- `LEADER_SYSTEM`：leader 只派活+综合的系统提示
- `WorkerTool`：`asyncio.create_task()` 异步派出，立即返回 worker_id；
  worker 跑完后往 leader mailbox 发 `<task-notification>`
- `TaskRegistry`：记录 worker_id → asyncio.Task 的映射

`loop.py`（小改）
- `run_agent_turn` 每轮开始前调 `inject_pending_notifications()`
  把 mailbox 里的消息包装成 `Message.user()` 注入 messages

`cli.py`（小改）
- 加 `coordinator` 子命令，组装 leader registry（含 `WorkerTool`）

`tool.py`（小改）
- `SendMessageTool`：给指定 agent_id 的 mailbox 发消息（M7 网状通信）

**做完的标准**：
- leader 派出 worker 后立刻拿到 `"worker w1 已派出"`，不阻塞
- worker 后台并行跑，完成后 `<task-notification>` 出现在 leader messages
- leader 对 worker 中间过程一无所知，只看到结论通知
- worker 调 `spawn_worker` → 返回越权错误字符串
- `A send_message B → B receive 被唤醒`（网状最小验证）

**学到什么**：
- 异步结果伪装成用户消息无缝融进 ReAct 主循环（不需要改循环核心）
- mailbox 极简（73 行）+ 任务注册表 = 完整异步协作基础
- Coordinator 星型（leader↔workers）vs SendMessage 网状（任意 Agent 互传）两种拓扑
