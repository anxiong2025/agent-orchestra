...
- **此刻还没有的**:多轮记忆(M2)、工具与想→做→看循环(M3/M4)。M1 的 `messages` 每次只有一条。
...
- **还没有的**:工具、想→做→看(M3 把这个对话循环升级成真正的 Agent 循环)。

---

### ⬜ M3 · 第一个工具:想 → 做 → 看(+ maxTurns)
...
flowchart TD
    ...
    CLI --> LOOP["编排层 · loop.py　★M3 升级<br/>想→做→看 + maxTurns 熔断"]
...
**③ 流程图 —— 想→做→看的循环 + maxTurns 熔断(M3 的核心)**
...
**这张图是 M3 的灵魂**:
- **想→做→看是个循环**:工具结果回灌后**再问一次模型**,模型基于新结果决定下一步——可能再调工具,也可能给最终答案。
...
```
阶段一(能聊天的单 Agent)
  M1 单轮真实对话 → M2 多轮记忆 → M3 第一个工具(想→做→看) → M4 多工具并发
阶段二(多 Agent 协作)
